...
if [[ -x .venv/bin/uvicorn ]]; then
  exec .venv/bin/uvicorn raw_indexer.api:app --host 127.0.0.1 --port 8000 --log-level info "$@"
elif command -v uvicorn &>/dev/null; then
  exec uvicorn raw_indexer.api:app --host 127.0.0.1 --port 8000 --log-level info "$@"
else
