...
confidence, condensed steps and synthetic project membership remain inspectable
in node details instead of requiring extra dash patterns or line colors.
Essential uses only real function markers, with condensed solid edges for omitted
intermediate functions. The project summit remains the one synthetic node.

The bundled build and five-project browser comparison passed after this change,
including two-pattern assertions before/after function selection and
